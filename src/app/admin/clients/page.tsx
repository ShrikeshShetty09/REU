...
const collectionId = process.env.NEXT_PUBLIC_APPWRITE_CLIENTS_COLLECTION_ID as string | undefined;
...
export default function AdminClientsPage() {
  const [items, setItems] = useState<ClientDoc[]>([]);
  ...
  return (
    <div className="space-y-6">
      <section className="space-y-2">
        ...
        <p className="text-xs text-slate-600">Manage the list of key clients and their logos.</p>
      </section>
      ...
      <section className="rounded-2xl border bg-white p-4 text-sm shadow-sm">
        ...
        <form onSubmit={handleCreate} className="mt-3 grid gap-3 md:grid-cols-[2fr,2fr,auto] md:items-end">
          ...
          <div className="space-y-1">
            <label className="block text-xs font-medium text-slate-700">Logo (optional)</label>
            <input
              type="file"
              accept="image/*"
      ...
      <section className="rounded-2xl border bg-white p-4 text-sm shadow-sm">
        <h2 className="text-sm font-semibold text-slate-900">Existing clients</h2>
        {items.length === 0 ? (
          <p className="mt-2 text-xs text-slate-500">No clients yet.</p>
        ) : (
            ...
            {items.map((item) => (
              <li key={item.$id} className="flex items-center justify-between py-2">
                <div>
                  ...
                  {item.logoFileId && <p className="text-[11px] text-slate-500">Logo URL stored</p>}
                </div>
                <button
                  type="button"
