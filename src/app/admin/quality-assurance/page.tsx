...
const collectionId = process.env.NEXT_PUBLIC_APPWRITE_QUALITY_COLLECTION_ID as string | undefined;
...
export default function AdminQualityPage() {
  const [items, setItems] = useState<QualityDoc[]>([]);
  ...
  return (
    <div className="space-y-6">
      <section className="space-y-2">
        <h1 className="text-xl font-semibold text-slate-900">Quality Assurance</h1>
        <p className="text-xs text-slate-600">Manage visuals shown on the Quality Assurance page.</p>
      </section>
      ...
      <section className="rounded-2xl border bg-white p-4 text-sm shadow-sm">
        <h2 className="text-sm font-semibold text-slate-900">Existing items</h2>
        {items.length === 0 ? (
            ...
            {items.map((item) => (
              <li key={item.$id} className="flex items-center justify-between py-2">
                <div>
                  ...
                  {item.imageUrl && (
                    <p className="text-[11px] text-slate-500">Image URL stored</p>
                  )}
                </div>
                <button
                  type="button"
