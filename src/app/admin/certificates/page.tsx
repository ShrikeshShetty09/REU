...
const collectionId = process.env.NEXT_PUBLIC_APPWRITE_CERTIFICATES_COLLECTION_ID as string | undefined;
...
export default function AdminCertificatesPage() {
  ...
  async function handleCreate(e: FormEvent) {
    ...
    try {
      ...
      if (file && bucketId && endpoint && projectId) {
  ...
  return (
    <div className="space-y-6">
      <section className="space-y-2">
        ...
        <p className="text-xs text-slate-600">Upload and manage certificates displayed on the Certificates page.</p>
      </section>

      <section className="rounded-2xl border bg-white p-4 text-sm shadow-sm">
        <h2 className="text-sm font-semibold text-slate-900">Add certificate</h2>
        <form onSubmit={handleCreate} className="mt-3 grid gap-3 md:grid-cols-[2fr,2fr,auto] md:items-end">
          ...
          <div className="space-y-1">
            <label className="block text-xs font-medium text-slate-700">File (image or PDF)</label>
            <input
              type="file"
      ...
      <section className="rounded-2xl border bg-white p-4 text-sm shadow-sm">
        <h2 className="text-sm font-semibold text-slate-900">Existing certificates</h2>
        {items.length === 0 ? (
          <p className="mt-2 text-xs text-slate-500">No certificates yet.</p>
        ) : (
            ...
            {items.map((item) => (
              <li key={item.$id} className="flex items-center justify-between py-2">
                <div>
                  ...
                  {item.imageUrl && <p className="text-[11px] text-slate-500">File URL stored</p>}
                </div>
                <button
                  type="button"
